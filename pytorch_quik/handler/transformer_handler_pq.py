# mostly taken from https://github.com/pytorch/serve/blob/master/examples/Huggingface_Transformers/Transformer_handler_generalized.py
# but added nested json to include prob_* for each class when using
...
class TransformersSeqClassifierHandler(BaseHandler, ABC):
    ...
    def inference(self, input_batch):
        ...
        if self.setup_config["mode"] == "sequence_classification":
            ...
            print("This is now the output size from the Seq classification model", predictions[0].size())
            print("This is now the output from the Seq classification model", predictions)

            num_rows, num_cols = predictions[0].shape
            # donchesworth: added lines to also pull tensor preds
